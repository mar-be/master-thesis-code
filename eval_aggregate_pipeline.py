...
def write_file(dir_path, backend, results, agg_results, sv_res_prob: List[np.ndarray], n_qubits: int, circuits, circuit_type, permute, shots):
    res_prob = [counts_to_probability(
        r.get_counts(), n_qubits) for r in results]
    agg_res_prob = [counts_to_probability(
        r.get_counts(), n_qubits) for r in agg_results]

    data = []
    n_circuits = len(circuits)
    for i in range(n_circuits):
        data.append({"circuit": circuits[i].qasm(), "sv-result": sv_res_prob[i].tolist(
        ), "result": res_prob[i].tolist(), "agg-result": agg_res_prob[i].tolist()})
...
if __name__ == "__main__":
    ...
    backend_names = ['ibmq_qasm_simulator', 'ibmq_athens',
                     'ibmq_santiago', 'ibmq_quito', 'ibmq_lima', 'ibmq_belem']
    ...
    for backend_data in backend_data_list:
        for circ in circuits:
            input_pipeline.put(QuantumExecutionJob(circuit=circ.measure_all(
    ...
    aggregator = Aggregator(
        input=input_pipeline, output=input_exec, job_dict=agg_job_dict, timeout=10)
    aggregator.start()
    ...
    result_analyzer = ResultAnalyzer(
        input=output_exec, output=output_pipline, output_agg=agg_results, output_part=None)
    result_analyzer.start()

    aggregator_results = AggregatorResults(
        input=agg_results, output=output_pipline, job_dict=agg_job_dict)
    ...
    for i in range(n_results):
        ...
        log.debug(
            f"{i}: Got job {job.id},type {job.type}, from backend {backend_name}, success: {r.success}")
        ...
        if len(results[backend_name]) == n_circuits and len(agg_results[backend_name]) == 0:
            log.info(
                f"All results for not aggregated circuits are available for backend {backend_name}")
        elif len(agg_results[backend_name]) == n_circuits:
            log.info(
                f"All results for aggregated circuits are available for backend {backend_name}")
            write_file(dir_path, backends[backend_name]["backend"], results.pop(backend_name), agg_results.pop(
